...
export async function sauvegarderMessage({
  ...
  id_whatsapp = null, repond_a_id_whatsapp = null,
  reference_fichier = null, reference_produit = null
}) {
  const { data: inserted, error } = await supabase
    .from('historique_messages')
    .insert({
      phone, session_id, role, content, type,
      id_whatsapp, repond_a_id_whatsapp, reference_fichier, reference_produit,
      statut: 'normal'
    })
    .select('id')
    .single()
